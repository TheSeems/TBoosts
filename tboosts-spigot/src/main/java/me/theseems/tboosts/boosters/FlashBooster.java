...
public abstract class FlashBooster implements TemporaryBooster, Booster {
  private BoosterMeta meta;
  private UUID player;

  public FlashBooster(UUID player) {
    this.player = player;
  }

  private float getMaxAmount() {
    final double[] maxAmount = {0.2};
    for (Booster booster : TBoostsAPI.getBoosterStorage().getBoosters(player)) {
      if (booster instanceof FlashBooster) {
        booster
                .getMeta()
                .getDouble("amount")
                .ifPresent(aDouble -> maxAmount[0] = Math.max(maxAmount[0], 0.2 * aDouble));
      }
    }
  ...
  public void apply() {
    ...
    if (actual != null) actual.setWalkSpeed(getMaxAmount());
  }
  ...
  public void take() {
    ...
    System.out.println("At the end max amount = " + getMaxAmount());
    actual.setWalkSpeed(getMaxAmount());
  }
  ...
  private void initMeta(UUID player, double amount) {
    ...
    if (!getMeta().getKeys().contains("display_name")) getMeta().set("display_name", "§b§lФлеш");
  }
